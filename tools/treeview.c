...
/////////////////////////////////////////////////
//                TREE VIEW                    //
//   This is a tool designed to view *.tree    //
//      files generated by this project.       //
/////////////////////////////////////////////////
// Building:                                   //
//  Ensure that the library ncurses is         //
//  is installed before building, the run the  //
//  command: make                              //
/////////////////////////////////////////////////
// Running:                                    //
//  Use the command                            //
//  ./treeview.o [filename.tree]               //
/////////////////////////////////////////////////
...
void statusbar(int count, char* filename, int width, int k);
...
int main(int argc, char *argv[])
{
  ...
  while (running)
  {
    int row = 0;
    drawTree(root, &row, 3);
    statusbar(node_count, argv[1], w.ws_col, ky);
    refresh();
    selectOption(row);
  }
  endwin();
...
void statusbar(int count, char* filename, int width, int k)
{
  ...
  mvprintw(rows_count - 1, cols_count - 8, "Key:%3d", k);
...
void drawNode(struct tree* node, int row, int col)
{
  if ((0 <= (row + offset)) && ((row + offset) < rows_count - 1))
  {
    ...
    if (row==sindex) 
    {
      if (node->childCount==0) mvprintw(sindex + offset, 0, "[*]");
      else if (node->show_c) mvprintw(sindex + offset, 0, "[-]");
      else if(!node->show_c) mvprintw(sindex + offset, 0, "[+]");
